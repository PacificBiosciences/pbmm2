  ...
  $ "$PBMM2" align -j 1 "$TESTDIR"/data/median.subreadset.xml "$REF" "$CRAMTMP"/out_sub.subreadset.xml --preset SUBREAD 2> "$CRAMTMP"/out_sub.err || echo $?
  1
  ...
  *Output has to be an alignment dataset! Please use alignmentset.xml, consensusalignmentset.xml, or transcriptalignmentset.xml!* (glob)
  ...
  $ "$PBMM2" align -j 1 "$TESTDIR"/data/median.subreadset.xml "$REF" "$CRAMTMP"/out_sub.alignmentset.xml --preset SUBREAD
