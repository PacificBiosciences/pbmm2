  ...
  $ ID=$(samtools view -H $CRAMTMP/split.3260208_188nM-GTAC_2xGCratio_LP7_100fps_15min_5kEColi_SP2p1_3uMSSB_BA243494.bam | grep 3260208_188nM-GTAC_2xGCratio_LP7_100fps_15min_5kEColi_SP2p1_3uMSSB_BA243494 | cut -f 2 | cut -f 2 -d ':')
  $ samtools view $CRAMTMP/split.3260208_188nM-GTAC_2xGCratio_LP7_100fps_15min_5kEColi_SP2p1_3uMSSB_BA243494.bam | grep -vc ${ID} | tr -d ' '
  0
  $ samtools view $CRAMTMP/split.3260208_188nM-GTAC_2xGCratio_LP7_100fps_15min_5kEColi_SP2p1_3uMSSB_BA243494.bam | wc -l | tr -d ' '
  9

  $ ID=$(samtools view -H $CRAMTMP/split.test_test.bam | grep test_test | cut -f 2 | cut -f 2 -d ':')
  $ samtools view $CRAMTMP/split.test_test.bam | grep -vc ${ID} | tr -d ' '
  0
  $ samtools view $CRAMTMP/split.test_test.bam | wc -l | tr -d ' '
  11

  $ ID=$(samtools view -H $CRAMTMP/split.UCLA_1023.bam | grep UCLA_1023 | cut -f 2 | cut -f 2 -d ':')
  $ samtools view $CRAMTMP/split.UCLA_1023.bam | grep -vc ${ID} | tr -d ' '
  0
  $ samtools view $CRAMTMP/split.UCLA_1023.bam | wc -l | tr -d ' '
  10
  ...
  $ grep "PacBio.AlignmentFile.AlignmentBamFile" $CRAMTMP/split_dataset.test_test.alignmentset.xml | tr -d '\t' | cut -f 3 -d ' ' | cut -f 2 -d '=' | tr -d '"'
  split_dataset.test_test.bam
  $ grep "PacBio.AlignmentFile.AlignmentBamFile" $CRAMTMP/split_dataset.UCLA_1023.alignmentset.xml | tr -d '\t' | cut -f 3 -d ' ' | cut -f 2 -d '=' | tr -d '"'
