  ...
  $ "$PBMM2" align "$MERGED" "$REF" "$CRAMTMP"/split.bam --split-by-sample
...
When both --split-by-sample and --sample were set, expect to see only one bam file with SM overridden.
  ...
  $ "$PBMM2" align "$REF" "$IN" "$CRAMTMP"/splitsampleoverride.consensusalignmentset.xml --sort -j 8 --split-by-sample --sample "MySample"  2>&1 | fgrep -v 'Requested more threads'
  ...
  *Offending bio sample names. BAM contains 'bamSample' and XML contains 'UCLA_1023'. Will ignore XML bio sample name.* (glob)
  ...
  $ "$PBMM2" align --split-by-sample "$NO_SM_BIOSAMPLES" "$REF" "$CRAMTMP"/split-no-sm.bam
