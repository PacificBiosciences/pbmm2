...
# "|SSS|" - Segmental duplication
  ...
  $ "$PBMM2" align "$REF" "$READS" "$CRAMTMP"/enforced0.bam -j 1 --log-level FATAL --enforced-mapping "$MAPPING" --preset CCS
  ...
  $ "$PBMM2" align "$CRAMTMP"/enforced1.mmi "$READS" "$CRAMTMP"/enforced1.bam -j 1 --log-level FATAL --enforced-mapping "$MAPPING" --preset CCS
