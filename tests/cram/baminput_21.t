  ...
  $ "$PBMM2" align -j 1 "$TESTDIR"/data/median.transcriptset.xml "$REF" "$CRAMTMP"/out_trans_upper.TranscriptAlignmentSet.XmL --preset ISOSEQ || echo $?
  $ "$PBMM2" align -j 1 "$TESTDIR"/data/median.transcriptset.xml "$REF" "$CRAMTMP"/out_json_upper.JsON --preset ISOSEQ || echo $?
  $ "$PBMM2" align -j 1 "$TESTDIR"/data/median.transcriptset.xml "$REF" "$CRAMTMP"/out_xml_upper.XML --preset ISOSEQ 2> "$CRAMTMP"/out_xml_upper.err  || echo $?
  1
  ...
  *Output is XML, but of unknown type! Please use alignmentset.xml, consensusalignmentset.xml, or transcriptalignmentset.xml* (glob)
  ...
  $ "$PBMM2" align "$IN" "$REF" "$CRAMTMP"/sorted.bam --preset SUBREAD --sort -j 2 -J 2 -m 100M --log-level INFO 2>&1| grep INFO
