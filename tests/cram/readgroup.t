  ...
  $ "$PBMM2" align -j 1 "$REF" "$FASTA" "$CRAMTMP"/rg.bam --log-level FATAL
  ...
  $ "$PBMM2" align -j 1 "$REF" "$FASTA" "$CRAMTMP"/rg.bam --log-level FATAL --rg '@RG'
  *Invalid @RG line. Missing ID field. Please provide following format: '@RG\tID:xyz\tSM:abc'* (glob)
  [1]

  $ "$PBMM2" align -j 1 "$REF" "$FASTA" "$CRAMTMP"/rg.bam --log-level FATAL --rg '@XY'
  *Invalid @RG line. Missing ID field. Please provide following format: '@RG\tID:xyz\tSM:abc'* (glob)
  [1]

  $ "$PBMM2" align -j 1 "$REF" "$FASTA" "$CRAMTMP"/rg.bam --log-level FATAL --sample newSampleName
  $ "$SAMTOOLS" view -H "$CRAMTMP"/rg.bam | grep "^@RG"
  @RG\tID:default\tPL:PACBIO\tDS:READTYPE=UNKNOWN\tPU:default\tSM:newSampleName\tPM:SEQUEL (esc)
  $ rm "$CRAMTMP"/rg.bam

  $ "$PBMM2" align -j 1 "$REF" "$FASTA" "$CRAMTMP"/rg.bam --log-level FATAL --rg '@RG\tID:myid'
  $ "$SAMTOOLS" view -H "$CRAMTMP"/rg.bam | grep "^@RG"
  @RG\tID:myid\tPL:PACBIO\tDS:READTYPE=UNKNOWN\tPU:default\tSM:UnnamedSample\tPM:SEQUEL (esc)
  $ rm "$CRAMTMP"/rg.bam

  $ "$PBMM2" align -j 1 "$REF" "$FASTA" "$CRAMTMP"/rg.bam --log-level FATAL --rg '@RG\tID:myid\tSM:mysample'
  $ "$SAMTOOLS" view -H "$CRAMTMP"/rg.bam | grep "^@RG"
  @RG\tID:myid\tPL:PACBIO\tDS:READTYPE=UNKNOWN\tPU:default\tSM:mysample\tPM:SEQUEL (esc)
  $ rm "$CRAMTMP"/rg.bam

  $ "$PBMM2" align -j 1 "$REF" "$FASTA" "$CRAMTMP"/rg.bam --log-level FATAL --rg '@RG\tID:myid\tSM:mysample' --sample newSampleName
  $ "$SAMTOOLS" view -H "$CRAMTMP"/rg.bam | grep "^@RG"
  @RG\tID:myid\tPL:PACBIO\tDS:READTYPE=UNKNOWN\tPU:default\tSM:newSampleName\tPM:SEQUEL (esc)
  $ rm "$CRAMTMP"/rg.bam

  $ "$PBMM2" align -j 1 "$REF" $BAM "$CRAMTMP"/rg.bam --log-level FATAL --rg '@RG\tID:myid'
