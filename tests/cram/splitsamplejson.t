  ...
  $ "$PBMM2" align -j 1 "$MERGED" "$REF" "$CRAMTMP"/split_dataset_sorted_json.alignmentset.xml --preset SUBREAD --split-by-sample
  ...
  $ ID=$(samtools view -F 4 -H "$CRAMTMP"/split_dataset_sorted_json.test_test.bam | grep "test test" | grep -v "@PG	ID:samtools" | cut -f 2 | cut -f 2 -d ':')
  ...
  $ ID=$(samtools view -F 4 -H "$CRAMTMP"/split_dataset_sorted_json.UCLA_1023.bam | grep "UCLA 1023" | grep -v "@PG	ID:samtools" | cut -f 2 | cut -f 2 -d ':')
