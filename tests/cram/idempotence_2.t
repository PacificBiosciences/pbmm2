  ...
  $ "$PBMM2" align -j 1 "$IN" "$REF" "$CRAMTMP"/sorted.bam --preset CCS --sort --log-level FATAL
  $ "$PBMM2" align -j 1 "$CRAMTMP"/sorted.bam "$REF" "$CRAMTMP"/idempotent.bam --preset CCS --sort --log-level FATAL
